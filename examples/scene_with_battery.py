...
class RobotWithBattery(Robot):
    def __init__(self, name, radius, path_executor, grasp_generator, battery_capacity, 
    consumption_rate):
        super().__init__(name=name, radius=radius, path_executor=path_executor, 
...
if __name__ == "__main__":
    ...
    first_plan = TaskPlan(actions=actions_first_plan)
    result, num_completed = robot.execute_plan(first_plan)

    first_plan_completed = True

        # Esegui il primo piano
    if first_plan_completed == True :
        
        # Aggiorna la batteria dopo ogni azione
        robot.update_battery(50)  # Supponiamo che ogni azione richieda 50 secondi
    ...
    # Controllo finale del livello della batteria dopo il primo piano
    print(f"Battery level after first plan: {robot.battery_level:.2f}")

    
    # Secondo piano di azioni
    actions_second_plan = [
        TaskAction(
            "navigate",
    ...
    second_plan = TaskPlan(actions=actions_second_plan)
    result, num_completed = robot.execute_plan(second_plan)
    second_plan_completed = True

    # Esegui il secondo piano
    if second_plan_completed == True:
    ...
    print("Second plan completed.")
        
    # Controllo finale del livello della batteria dopo il secondo piano
    print(f"Final battery level: {robot.battery_level:.2f}")
    # Aggiorna la batteria (ipoteticamente carica o parzialmente carica)
    robot.battery_level = robot.battery_capacity
    print(f"Battery recharged: {robot.battery_level:.2f}")
