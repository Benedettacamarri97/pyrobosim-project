...
def create_world(multirobot=True):
    ...
    world.add_object(name="inventario_consegne",category="banana", parent=counter)
...
def pick_and_place(robot, obj_name, world):
    ...
    if obj_info:
        ...
        # Verifica se il robot si trova già nella stessa location dell'oggetto
        if robot.location != obj_location:
            print(f"{robot.name} naviga verso {obj_name} alla location {obj_location.name}.")
            navigate_to_object = TaskAction("navigate", source_location=robot.location.name, target_location=obj.parent.name)
            robot.execute_action(navigate_to_object)
        
        print(f"{robot.name} is attempting to pick up {obj_name}.")
        robot.pick_object(obj_name)
        
        print(f"{robot.name} is navigating to the destination {destination}.")
        navigate_to_dest = TaskAction("navigate", source_location=obj.parent.name, target_location=destination)
        result = robot.execute_action(navigate_to_dest)
        if not result:
            print(f"Navigazione fallita verso {destination}. Verifica la presenza di ostacoli o la definizione della destinazione.")
            return

        
        print(f"{robot.name} sta spostando {obj_name} ({obj_category}) alla {destination}") #
        place = TaskAction(
            "place",
        )
        #navigate_to_room(robot, destination, world)
        robot.execute_action(place) #

        # Se la destinazione è un altro robot
        if destination in world.get_robot_names(): #
            other_robot = world.get_robot_by_name(destination) #
            print(f"{robot.name} consegna {obj_name} ({obj_category}) al robot {other_robot.name}")
            navigate_to_robot(robot, other_robot.name, world)
            other_robot.pick_object(obj_name)
            robot.execute_action(place)

    else:
        print(f"Oggetto {obj_name} non trovato nel dizionario.")
# Ad esempio: robot0 prende un pacchetto e lo porta alla delivery_area
#pick_and_place(robot0, "pacchetto", "banana", world)
...
if __name__ == "__main__":
    ...
    pick_and_place(robot1, "pacco_grande",  world)   # Robot 1 consegna uno scatolone
